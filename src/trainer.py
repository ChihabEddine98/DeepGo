    ...
    def train(self):
		...
		if epoch < 150 : 
            ...
            if (epoch % self.config.print_frq == 0):
                ...
                console.print(title)
                val_hist.append(val)
                self.model.save(f"12LR_inception_se_192_sw{self.config.start_epoch}_to_{self.config.end_epoch}_{self.model_path}")       
        title = Markdown(f"## END of Training Saving Last [DGM]...", style=self.config.succes_style)
        console.print(title)
        histories['val_history'] = val_hist

        with open(f"{self.hist_path}_12LR_inception_se_192_sw_{self.config.end_epoch}", 'wb') as f_hist:
