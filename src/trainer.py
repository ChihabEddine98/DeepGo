    ...
    def train(self):
        ...
        for epoch in range (self.config.start_epoch, self.config.end_epoch + 1):
            ...
            if (epoch % self.config.print_frq == 0):
                ...
                self.model.save(f"mnet_256_21_384_525_sw_{self.config.start_epoch}_to_{self.config.end_epoch}_{self.model_path}")                  
                with open(f"{self.hist_path}_mnet_256_21_384_525_sw_{self.config.start_epoch}_to_{self.config.end_epoch}",'wb') as f_hist:
