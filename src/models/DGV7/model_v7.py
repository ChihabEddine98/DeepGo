...
class DGMV8(DGM):
    ...
    def inception_block(self,x,filters,kernels,pad='same'):
        ...
        t3 = layers.Conv2D(filters[4],kernels[2],padding=pad)(t3)
        t3 = self.activation(t3)
        
        t4 = layers.MaxPool2D(kernels[0],padding=pad)(x) 
    ...
    def output_policy_block(self,x):
        policy_head = layers.Conv2D(1, 1, padding='same', use_bias=1, kernel_regularizer=self.l2_reg)(x)
