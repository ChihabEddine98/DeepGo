...
class DGM(object):
    ...
    def __init__(self,version=0,dim=config.dim,n_moves=config.n_moves,n_planes=config.n_planes,
                n_filters=config.n_filters,kernel_size=config.kernel,l2_reg=config.l2_reg
                ,dropout=config.dropout_ratio,n_res_blocks=config.n_res_blocks) -> None:
        super().__init__()
    ...
    def output_value_block(self,x):
        ...
        value_head = layers.Dense(self.n_filters//2, kernel_regularizer=self.l2_reg)(value_head)
