...
class DGMV9_2(DGM):
    ...
    def __init__(self,version=92,n_filters=config.n_filters,kernel_size=config.kernel,l2_reg=config.l2_reg
    ...
    def mb_conv(self,x,se=1,k=3,r=4,pad='same'):
        ...
        if se:
            x_ = self.se_block(x_,ratio=r)
        
        x_ = layers.Conv2D(self.squeeze, 1,kernel_regularizer=self.l2_reg,use_bias=0)(x_)
    ...
    def se_block(self,x,ratio=4):
        ...
        x_ = layers.Dense(self.n_filters,kernel_regularizer=self.l2_reg,use_bias=1,activation='sigmoid')(x_)
        return layers.Multiply()([x_, x])
